...
class PensamPipeline:

    # Database setup
    conn = sqlite3.connect('pensam.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `pensam`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM pensam WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO pensam (date, title, link, content)"
